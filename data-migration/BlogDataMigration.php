...
class BlogDataMigration
{
    ...
    public function importDataFromExcel($file, $offset = null, $length = null)
    {
        ...
        $inputFileNameCatogeries = CBXPHPSPREADSHEET_ROOT_PATH . 'data-migration/excel/SwimlaneLiveBlogCategories.xlsx';

        $excel_data_array = $this->data_migration_helper->excelToArray($file);

        $excel_data_array_users = $this->data_migration_helper->excelToArray($inputFileNameUsers);
        $excel_data_array_catogeries = $this->data_migration_helper->excelToArray($inputFileNameCatogeries);
...
//exit();
        if (is_array($excel_data_array) && !empty($excel_data_array)) {
            ...
            foreach ($excel_data_array as $row) { 
                ...
                $post_image_id = str_replace($searchVal,$replaceVal,$row['blogimage']); // need to do

                $post_status = ($row['status']=="live")?"publish":"draft";
                $post_slug =  $row['slug'];

                $post_author_id = $row['authorid'];
                if (is_array($excel_data_array_users) && !empty($excel_data_array_users)) {
                    foreach ($excel_data_array_users as $rowa) { 
                ...
                $resource_type = array(37);

                $searchVal = ['["','"]']; $replaceVal = ['',''];
                $blog_tags = str_replace($searchVal,$replaceVal,$row['tags']); // need to do
                
                $blog_tags_count = explode(',' , $blog_tags);
                if($blog_tags_count > 0){
                    $searchVal = ['","']; $replaceVal = [','];
                    $blog_tags = str_replace($searchVal,$replaceVal,$blog_tags);
                }
            
                $searchVal = ['["','"]']; $replaceVal = ['',''];
                $blog_categories = str_replace($searchVal,$replaceVal,$row['categories']); // need to do
                
                $blog_categories_count = explode(',' , $blog_categories);
                if($blog_categories_count > 0){
                    $searchVal = ['","']; $replaceVal = [','];
                    $blog_categories = str_replace($searchVal,$replaceVal,$blog_categories);
                }
                
                $blog_categories_array = [];
                $blog_categories = explode(',', $blog_categories);
                if (is_array($excel_data_array_catogeries) && !empty($excel_data_array_catogeries)) { 
                    foreach ($excel_data_array_catogeries as $rowc) { 
                        if(in_array($rowc['id'], $blog_categories)){ //var_dump($row['title']);
                            ...
                            if ( $term == 0 && $term == null ) {
                                ...
                                if($insertedTerm) {
                                    ...
                                    $blog_categories_array[] = $insertedTerm['term_id'];
                                }
                            }else{
                                $blog_categories_array[] = $term['term_id'];
                            }
                        }
                    }
                }

              //  $meta_title = $row['yoast_wpseo_title']; // need to do
             //   $meta_description = $row['yoast_wpseo_metadesc']; // need to do
                  ...
                  //  $published_count++;
                    ...
                    $post_data = array(
                        ...
                        'tax_input' => array(
                            ...
                            'blog-category' => $blog_categories_array,
                        ),
                        'meta_input' => array(
                    ...
                    if (is_wp_error($result)) {
                        ...
                        // $featured_img_id = $this->attachImage($post_image_id, $post_id);
                        // $feature_img_url = '';
                        // if ($featured_img_id) {
    ...
    /**
     * Update posts images from csv data
     * @param $file
     * @param null $offset
     * @param null $length
     * @return string
     */
    public function importImagesFromExcel($file)
    {
        $message = '';
        $excel_data_array_images = $this->data_migration_helper->excelToArray($file);
        //need to do
